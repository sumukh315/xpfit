...
export default function WorkoutImport() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-6">
      <div className="mb-6">
        <h1 className="pixel-font text-purple-400 mb-1" style={{ fontSize: '14px' }}>Import Workout History</h1>
      ...
      <div className="flex gap-2 mb-6">
        {[{ key: 'csv', label: '📄 CSV Upload' }, { key: 'text', label: '📋 Paste / Text' }].map(t => (
          <button key={t.key} onClick={() => { setTab(t.key); setImportResult(null); setImportError('') }}
            className={`pixel-btn py-2 px-5 ${tab === t.key ? 'bg-purple-800 border-purple-500 text-white' : 'bg-gray-900 border-gray-700 text-gray-400'}`}
            style={{ fontSize: '9px' }}>
            {t.label}
          </button>
        ))}
      ...
      {tab === 'csv' && (
        <div>
          <div className="pixel-card p-5 mb-4">
            ...
            <div
              className="border-2 border-dashed border-gray-700 hover:border-purple-500 transition-colors rounded p-8 text-center cursor-pointer"
        ...
        <div>
          <div className="pixel-card p-5 mb-4">
            ...
            <textarea
              ...
              rows={10}
              className="w-full bg-black/40 border-2 border-gray-700 text-white px-3 py-2 focus:border-purple-500 outline-none font-mono resize-y"
              style={{ fontSize: '12px' }}
            ...
            <button onClick={handleParseText} disabled={!pasteText.trim()}
              className="pixel-btn bg-purple-800 border-purple-600 text-white px-6 py-2 mt-3 disabled:opacity-40"
              style={{ fontSize: '9px' }}>
              Parse Text
            </button>
          </div>
      ...
      {parsedWorkouts && parsedWorkouts.length > 0 && (
        <div className="pixel-card p-5">
          <div className="flex justify-between items-center mb-4">
            <h2 className="pixel-font text-purple-400" style={{ fontSize: '11px' }}>
